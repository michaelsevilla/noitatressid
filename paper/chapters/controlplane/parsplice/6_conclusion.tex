...
problem. To show the generality of the Mantle API, we show how to to set
policies for applications in two orthogonal domains. First, we design load
balancing policies for ZLog sequencers and explore three policies that leverage
workload characteristics for improved performance: balancing modes, migration
units, and backoff. Next, we iterate to a custom solution for our molecular
...
works for a variety of scenarios, we instead use the Mantle framework to enable
